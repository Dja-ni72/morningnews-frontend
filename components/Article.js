...
function Article(props) {
	...
	const handleBookmarkClick = () => {
		...
		fetch(`http://localhost:3000/users/canBookmark/${user.token}`)
			...
			if (hiddenArticles.includes(props.title)) {
				return null; 
			  }
	}
	...
	return (
		<div className={styles.articles}>
			<div className={styles.articleHeader}>
				...
				<div>
				<FontAwesomeIcon onClick={() => dispatch(hideArticle(props.title))} icon={faEyeSlash} style={iconStyle} className={styles.bookmarkIcon} />
				</div>
			</div>
